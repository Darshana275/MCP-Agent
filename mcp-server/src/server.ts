...
app.post("/tool/github_scan", async (req, res) => {
  try {
    ...
    const branch: string = parsed.branch ?? "main";
    ...
    const depFiles = files.filter((f: string) =>
      interesting.some((n) => f.toLowerCase().endsWith(n))
    );
    ...
    const fetchFile = async (path: string) => {
      ...
      const content = Buffer.from((resp.data as any).content, "base64").toString("utf8");
      return { path, content };
    };

    const depContents = await Promise.all(depFiles.map(fetchFile));

    const secretHits = files.filter((p: string) => /(\.env|id_rsa|\.pem|secret|key)/i.test(p));
    const anomalyHits = files.filter((p: string) =>
      /(^|\/)(\.(github|gitlab)\/workflows\/.*\.yml$)/i.test(p)
    );
...
app.post("/tool/risk_score", async (req, res) => {
  try {
    ...
    const heuristicScore = (pkg: string) => {
      ...
      if (highRiskPatterns.some((w) => name.includes(w))) return 9;
      if (deprecated.includes(name)) return 8;
      if (mediumRisk.includes(name)) return 5;
      return 2;
    };
    ...
    type RiskRow = {
      ...
      osv?: { id: string; summary?: string; severity?: number; url?: string }[];
      ecosystem?: "npm" | "PyPI";
    };
    ...
    const scoreOne = async (pkg: string, eco: "npm" | "PyPI") => {
      // heuristic baseline
      ...
      if (osv.vulnerable) {
        ...
        const sevBump = Math.max(
          0,
          ...osv.vulns.map((v) => (v.severity ? Math.min(3, Math.floor(v.severity / 3)) : 0))
        ); // adds up to +3 from severity buckets
    ...
    for (const pkg of uniqNpm) tasks.push(limit(() => scoreOne(pkg, "npm")));
    for (const pkg of uniqPip) tasks.push(limit(() => scoreOne(pkg, "PyPI")));
    ...
    const worst = results.reduce(
      ...
      { package: "", score: 0, level: "Low" as const }
    );
...
app.post("/tool/recommend_actions", async (req, res) => {
  try {
    ...
    else actions.push({ type: "PASS", message: "Low risk, safe to proceed." });
